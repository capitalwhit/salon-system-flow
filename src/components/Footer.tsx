          ...
          <nav className="flex flex-wrap justify-center gap-6 md:gap-8">
            {header.navItems.map((item) =>
              item.href.startsWith("/") ? (
                <Link key={item.href} to={item.href} className="text-muted-foreground hover:text-primary transition-colors">
                  {item.label}
                </Link>
              ) : (
            ...
            <Link to="/work-with-us" className="text-muted-foreground hover:text-primary transition-colors">Contact</Link>
          </nav>
        </div>
